...
def parse_and_generate_err_files(input_file_path, output_dir="."):
    """
    解析指定的输入文件，并为每个查询生成独立的.err文件
    如果一个查询模板只有一个index，则命名为queryname.err
    如果一个查询模板有多个index，则命名为queryname_index.err
    ...
    for block in file_blocks:
        ...
        for i in range(1, len(index_blocks)):
            ...
            # 提取Log view和Cost time
            logview_match = re.search(r'Log view:\n(https://.*)', index_block)
            logview = logview_match.group(1).strip() if logview_match else "N/A"

            cost_time_match = re.search(r'Cost time\s*:\s*(\d+)\s*ms', index_block)
            cost_time = cost_time_match.group(1).strip() if cost_time_match else "N/A"

            # 新增：提取starttime和endtime
            starttime_match = re.search(r'starttime:([^\s,]+ [^\s,]+)', index_block)
            starttime = starttime_match.group(1).strip() if starttime_match else "N/A"

            endtime_match = re.search(r'endtime:([^\s,]+ [^\s,]+)', index_block)
            endtime = endtime_match.group(1).strip() if endtime_match else "N/A"
    ...
    for data in query_data:
        ...
        if query_name_count[query_name] == 1:
            # 只有一个index，使用queryname.err格式
            output_filename = "{}.err".format(query_name)
        else:
            # 有多个index，使用queryname_index.err格式
            output_filename = "{}_{}.err".format(query_name, index)

        output_filepath = os.path.join(output_dir, output_filename)

        # 写入.err文件（兼容Python 2和3）
        if sys.version_info[0] == 3:
            # Python 3
            with open(output_filepath, 'w', encoding='utf-8') as f:
                ...
                f.write("End time: {}\n".format(data['endtime']))
                f.write("Cost time: {} ms\n".format(data['cost_time']))
        else:
            # Python 2
            import codecs
...
def main():
    ...
    主函数，支持命令行参数
    """
    parser = argparse.ArgumentParser(description='解析TPC-DS基准测试输出文件，为每个查询生成独立的.err结果文件')
    parser.add_argument('input_file', help='输入文件路径 (例如: err.out)')
    parser.add_argument('-o', '--output', default='query_results', help='输出目录路径 (默认: query_results)')

    args = parser.parse_args()

    try:
        parse_and_generate_err_files(args.input_file, args.output)
...
if __name__ == "__main__":
    # 可以直接调用函数指定参数
    # parse_and_generate_err_files("err.out", "query_results")
