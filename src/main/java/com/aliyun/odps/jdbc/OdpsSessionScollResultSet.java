...
public class OdpsSessionScollResultSet extends OdpsResultSet implements ResultSet  {
  ...
  OdpsSessionScollResultSet(OdpsStatement stmt, OdpsResultSetMetaData meta, DownloadSession session)
  ...
  private void fetchRows() throws SQLException {
    ...
    try {
      ...
      TunnelRecordReader reader = sessionHandle.openRecordReader(cachedUpperRow, count, -1);
      ...
      conn.log.debug(String.format("fetch records, start=%d, cnt=%d, %d KB, %.2f KB/s", cachedUpperRow,
